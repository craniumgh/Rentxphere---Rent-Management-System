...
class Action
{
	...
	function save_user()
	{
		...
		// Ensure that the 'establishment_id' column is no longer used.
		$name = $this->db->real_escape_string($name);
		$username = $this->db->real_escape_string($username);
		$password = !empty($password) ? md5($this->db->real_escape_string($password)) : '';
		$type = (int)$type;
		...
		// Insert user without 'establishment_id'
		if (empty($id)) {
			...
			// Update user without 'establishment_id'
		...
		if ($stmt->affected_rows > 0) {
			...
			return 'No changes made or failed to save user.';
		}
	}
	...
	function get_tdetails()
	{
		extract($_POST);
		$data = array();
		$tenants = $this->db->query("SELECT t.*,concat(t.lastname,', ',t.firstname,' ',t.middlename) as name,h.house_no,h.price
FROM tenants t inner join houses h on h.id = t.house_id where t.id = {$id} ");
		...
		$last_payment = $this->db->query("SELECT * FROM payments where id != '$pid' and tenant_id =" . $id . " order by
unix_timestamp(date_created) desc limit 1");
		$paid = $paid->num_rows > 0 ? $paid->fetch_array()['paid'] : 0;
		$data['paid'] = number_format($paid, 2);
		$data['last_payment'] = $last_payment->num_rows > 0 ? date(
			"M d, Y",
			strtotime($last_payment->fetch_array()['date_created'])
		) : 'N/A';
